# pred_measure_wrappers.R: user-facing convenience wrappers for direct
# metric and score computation.
...
mse <- function(y, mupred, log_weights) {
  n_obs <- length(y)
  n_draws <- nrow(mupred)
  checkmate::assert_matrix(mupred, ncols = n_obs)
...
elpd <- function(ylp, log_weights) {
  n_obs <- ncol(ylp)
  n_draws <- nrow(ylp)
  checkmate::assert_matrix(ylp, ncols = n_obs)
  checkmate::assert_matrix(log_weights, nrows = n_draws, ncols = n_obs)
...
rps <- function(y, ypred, log_weights) {
  n_obs <- length(y)
  n_draws <- nrow(ypred)
  checkmate::assert_matrix(ypred, ncols = n_obs)
  checkmate::assert_matrix(log_weights, nrows = n_draws, ncols = n_obs)
